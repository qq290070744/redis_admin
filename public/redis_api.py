...
def cluster_connect(conf, username=None, password=None):
    logs.debug('redis cluster connect:{0},username:{1},password:{2}'.format(conf, username, password))
    return StrictRedisCluster(startup_nodes=conf, decode_responses=True,
    ...
    if name is None and user is not None:
        ...
        try:
            ...
            for cluster in cluster_confs:
                ...
                cluster_conf_dict['username'] = cluster.username
